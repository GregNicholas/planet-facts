...
const InfoItem = styled.li`
  ...
  @media (min-width: 768px) {
    ...
    height: 2.5rem;
    border: ${(props) =>
      !props.selected ? "1px solid rgba(255, 255, 255, .2)" : "none"};
    background: ${(props) => (!props.selected ? "none" : props.color)};
    opacity: 100%;
    
    &:hover {
        background: ${(props) => (!props.selected && "rgba(255, 255, 255, .2)")};
    }
  }
  @media (min-width: 1024px) {
    height: 3rem;
...
@media (min-width: 768px) and (max-width: 1024px) {
  width: ${(props) => `${parseInt(props.width.tablet.slice(0,props.width.tablet.length-2))/2}px`};
}
@media (min-width: 1024px) {
  width: ${(props) => `${parseInt(props.width.desktop.slice(0,props.width.desktop.length-2))/2}px`};
}
  position: absolute;
  left: 50%;
  bottom: -15%;
...
const PlanetPage = ({ planet }) => {
  console.log("IMAGES: ", planet.images, planet.images.geology.substring(1));
  ...
  return (
    <Container>
        ...
        <InfoList>
          ...
          >
            {screenWidth < 768 ? (
              ...
              <>
                <StatIndex>01</StatIndex> <span>overview</span>
              </>
            )}
          </InfoItem>
          <InfoItem
            selected={info === "structure"}
            color={planet.color}
            onClick={() => {
          ...
          >
            {screenWidth < 768 ? (
              ...
              <>
                <StatIndex>02</StatIndex> <span>internal structure</span>
              </>
            )}
          </InfoItem>
          <InfoItem
            selected={info === "geology"}
            color={planet.color}
            onClick={() => {
              setImage(planet.images.planet.substring(1));
              setInfo("geology");
          ...
          >
            {screenWidth < 768 ? (
              ...
              <>
                <StatIndex>03</StatIndex> <span>surface geology</span>
              </>
            )}
          </InfoItem>
        </InfoList>
      ...
      <PlanetInfo>
        ...
        <Source>
          Source :{" "}
          <WikiLink href={planet[info].source} target="_blank">
      ...
      <Stats>
        <StatItem>
          ...
          <Stat>{planet.rotation}</Stat>
        </StatItem>
        <StatItem>
          <StatName>revolution time</StatName>
          <Stat>{planet.revolution}</Stat>
        </StatItem>
        <StatItem>
          <StatName>radius</StatName>
          <Stat>{planet.radius}</Stat>
        </StatItem>
        <StatItem>
          <StatName>average temp.</StatName>
          <Stat>{planet.temperature}</Stat>
        </StatItem>
      </Stats>
    </Container>
  );
